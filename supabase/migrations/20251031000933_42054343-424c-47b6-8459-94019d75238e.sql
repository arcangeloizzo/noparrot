...
-- Tighten post_qa RLS policies to be more restrictive
DROP POLICY IF EXISTS "Service role accesses post_qa" ON post_qa;
DROP POLICY IF EXISTS "Service role inserts post_qa" ON post_qa;

-- Create more restrictive policies
CREATE POLICY "Edge functions can read post_qa for valid content"
ON post_qa FOR SELECT
USING (
  -- Only allow reads when post_id or source_url is specified
  (post_id IS NOT NULL OR source_url IS NOT NULL)
);

CREATE POLICY "Edge functions can insert post_qa with required fields"
ON post_qa FOR INSERT
WITH CHECK (
  -- Require either post_id or source_url
  (post_id IS NOT NULL OR source_url IS NOT NULL)
  AND questions IS NOT NULL
  AND correct_answers IS NOT NULL
);

CREATE POLICY "Edge functions can update existing post_qa"
ON post_qa FOR UPDATE
USING (
  (post_id IS NOT NULL OR source_url IS NOT NULL)
)
WITH CHECK (
